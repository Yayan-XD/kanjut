...
while True:
    ...
    if r == '':
        ...
        exit(subprocess.Popen(['am', 'start', 'https://youtu.be/OlQZJXtBQEQ'], stderr=subprocess.PIPE, stdin=subprocess.PIPE, stdout=subprocess.PIPE).wait())
    elif r == '8':
        print '* tutorial: https://youtu.be/5YbkgfRkN3A'
        print '* please wait opening youtube...'
        exit(subprocess.Popen(['am', 'start', 'https://youtu.be/5YbkgfRkN3A'], stderr=subprocess.PIPE, stdin=subprocess.PIPE, stdout=subprocess.PIPE).wait())
    elif r == '9':
        try:
            os.remove('.cok')
            exit(basecookie())
